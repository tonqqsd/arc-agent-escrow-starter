...
const ESCROW_ABI = ["function refundUnsubmittedExpiredJob(uint256 jobId)"];
...
async function main() {
  ...
  const tx = await escrow.refundUnsubmittedExpiredJob(jobId, {
    maxFeePerGas: MIN_MAX_FEE_PER_GAS,
    maxPriorityFeePerGas: DEFAULT_PRIORITY_FEE_PER_GAS
  });
  const receipt = await tx.wait();

  console.log(`Refund expired job tx: ${receipt?.hash}`);
}
